const form = document.getElementById('calculatorForm');
const steps = [...document.querySelectorAll('.step')];
const backBtn = document.getElementById('backBtn');
const nextBtn = document.getElementById('nextBtn');
const formError = document.getElementById('formError');
const progressFill = document.getElementById('progressFill');
const stepLabel = document.getElementById('stepLabel');
const resultsSection = document.getElementById('results');
const resultRange = document.getElementById('resultRange');
const resultExplanation = document.getElementById('resultExplanation');
const resultSummary = document.getElementById('resultSummary');

let currentStep = 0;
const totalSteps = steps.length;

const baseRanges = {
  Cosmetic: [8000, 15000],
  Standard: [15000, 25000],
  Full: [25000, 45000],
};

const multipliers = {
  bathroomType: {
    'Powder Room': 0.85,
    'Guest Bathroom': 1,
    'Primary Bathroom': 1.25,
  },
  layoutChanges: {
    'No layout changes': 1,
    'Minor layout changes': 1.12,
    'Major layout changes': 1.25,
  },
  scope: {
    'Surface updates': 0.95,
    'Partial gut': 1.08,
    'Full gut': 1.2,
  },
  finishLevel: {
    Value: 0.95,
    'Mid-range': 1.08,
    Premium: 1.2,
  },
  timeline: {
    ASAP: 1.12,
    '1-3 months': 1.05,
    '3-6 months': 1,
  },
};

function showStep(index) {
  steps.forEach((step, idx) => step.classList.toggle('active', idx === index));

  backBtn.style.visibility = index === 0 ? 'hidden' : 'visible';
  nextBtn.textContent = index === totalSteps - 1 ? 'See My Estimate' : 'Next';

  const progress = ((index + 1) / totalSteps) * 100;
  progressFill.style.width = `${progress}%`;
  stepLabel.textContent = `Step ${index + 1} of ${totalSteps}`;
  formError.textContent = '';
}

function getFieldValue(name) {
  const selected = form.querySelector(`[name="${name}"]:checked`);
  return selected ? selected.value : '';
}

function validateCurrentStep() {
  const activeStep = steps[currentStep];
  const requiredInputs = [...activeStep.querySelectorAll('[required]')];

  for (const input of requiredInputs) {
    if (input.type === 'radio') {
      if (!activeStep.querySelector(`[name="${input.name}"]:checked`)) {
        formError.textContent = 'Please select one option to continue.';
        return false;
      }
      continue;
    }

    if (!input.value.trim()) {
      formError.textContent = 'Please complete all fields to continue.';
      return false;
    }

    if (input.type === 'email' && !input.checkValidity()) {
      formError.textContent = 'Please enter a valid email address.';
      return false;
    }
  }

  return true;
}

function currency(value) {
  return new Intl.NumberFormat('en-US', {
    style: 'currency',
    currency: 'USD',
    maximumFractionDigits: 0,
  }).format(value);
}

function calculateEstimate() {
  const projectLevel = getFieldValue('projectLevel');
  const [baseMin, baseMax] = baseRanges[projectLevel];

  let totalMultiplier = 1;
  for (const [field, map] of Object.entries(multipliers)) {
    const selected = getFieldValue(field);
    if (selected && map[selected]) {
      totalMultiplier *= map[selected];
    }
  }

  const budget = getFieldValue('budget');
  if (budget === 'Under $15k') {
    totalMultiplier *= 0.92;
  } else if (budget === '$25k+') {
    totalMultiplier *= 1.12;
  }

  const min = Math.round((baseMin * totalMultiplier) / 500) * 500;
  const max = Math.round((baseMax * totalMultiplier) / 500) * 500;
  const cappedMax = projectLevel === 'Full' ? `${currency(max)}+` : currency(max);

  const explainer = `${projectLevel} projects in DFW generally start around ${currency(
    baseMin
  )} and can rise based on size, scope, and finish quality. Based on your selections, a realistic planning range is ${currency(
    min
  )} to ${cappedMax}.`;

  const selections = {
    Bathroom: getFieldValue('bathroomType'),
    'Layout impact': getFieldValue('layoutChanges'),
    Scope: getFieldValue('scope'),
    Finishes: getFieldValue('finishLevel'),
    Timeline: getFieldValue('timeline'),
    'Stated budget': budget,
  };

  return {
    displayRange: `${currency(min)} – ${cappedMax}`,
    explainer,
    selections,
  };
}

function renderResults() {
  const estimate = calculateEstimate();

  resultRange.textContent = estimate.displayRange;
  resultExplanation.textContent = estimate.explainer;

  resultSummary.innerHTML = '';
  Object.entries(estimate.selections).forEach(([label, value]) => {
    const item = document.createElement('li');
    item.textContent = `${label}: ${value}`;
    resultSummary.appendChild(item);
  });

  form.classList.add('hidden');
  resultsSection.classList.remove('hidden');
}

nextBtn.addEventListener('click', () => {
  if (!validateCurrentStep()) return;

  if (currentStep === totalSteps - 1) {
    renderResults();
    return;
  }

  currentStep += 1;
  showStep(currentStep);
});

backBtn.addEventListener('click', () => {
  if (currentStep === 0) return;
  currentStep -= 1;
  showStep(currentStep);
});

showStep(currentStep);
